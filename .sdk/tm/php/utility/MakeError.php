...
class HackernewsMakeError
{
    public static function call(?HackernewsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HackernewsContext([], null);
        }
        $op = $ctx->op ?? new HackernewsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HackernewsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HackernewsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HackernewsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof HackernewsError) {
