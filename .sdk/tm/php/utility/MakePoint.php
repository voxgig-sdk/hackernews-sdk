...
// Hackernews SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class HackernewsMakePoint
{
    public static function call(HackernewsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof HackernewsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = HackernewsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = HackernewsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
