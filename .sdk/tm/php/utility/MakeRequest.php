...
class HackernewsMakeRequest
{
    public static function call(HackernewsContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new HackernewsResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new HackernewsResponse($fetched);
        } else {
