...
//
// NewHackernewsSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.HackernewsSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewHackernewsSDK(opts)
}
...
func fcCandidates(client *sdk.HackernewsSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.HackernewsSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.HackernewsSDK, name string) any {
