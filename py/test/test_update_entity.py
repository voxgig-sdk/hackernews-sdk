...
class TestUpdateEntity:
    ...
    def test_should_stream(self):
        ...
        base = HackernewsSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HACKERNEWS_TEST_UPDATE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        update_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.update")))
        update_ref01_data = None
        if len(update_ref01_data_raw) > 0:
            update_ref01_data = helpers.to_map(update_ref01_data_raw[0][1])
...
def _update_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("HACKERNEWS_TEST_UPDATE_ENTID"))
