...
class HackernewsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HackernewsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HackernewsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HackernewsTestRunner.getenv("HACKERNEWS_TEST_LIVE")
        override = HackernewsTestRunner.getenv("HACKERNEWS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HackernewsTestRunner.getenv(key)
        ...
        explain = HackernewsTestRunner.getenv("HACKERNEWS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HACKERNEWS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HackernewsTestRunner._test_control is not None:
            return HackernewsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HackernewsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HackernewsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HackernewsTestRunner.load_test_control()
...
def load_env_local():
    HackernewsTestRunner.load_env_local()


def env_override(m):
    return HackernewsTestRunner.env_override(m)


def entity_data(v):
    return HackernewsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HackernewsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HackernewsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HackernewsTestRunner.load_test_control()


def live_client_options():
    return HackernewsTestRunner.live_client_options()


def live_delay_ms():
    return HackernewsTestRunner.live_delay_ms()
