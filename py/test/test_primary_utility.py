...
from hackernews_sdk.utility.voxgig_struct import voxgig_struct as vs
from hackernews_sdk import HackernewsSDK
from hackernews_sdk.core.spec import HackernewsSpec
from hackernews_sdk.core.result import HackernewsResult
from hackernews_sdk.core.response import HackernewsResponse
from hackernews_sdk.core.operation import HackernewsOperation
from hackernews_sdk.core.error import HackernewsError
from hackernews_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return HackernewsError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = HackernewsSDK.test(None, None)
        ...
        class TestInitFeature(HackernewsBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(HackernewsBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = HackernewsSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = HackernewsSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = HackernewsSDK.test(None, setup_opts)
