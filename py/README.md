...
The Python SDK for the Hackernews API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Item(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Hackernews(None).load(
    {"id": "test01"}, None
)
