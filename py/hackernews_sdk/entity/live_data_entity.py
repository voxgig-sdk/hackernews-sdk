...
from hackernews_sdk.hackernews_types import (
    LiveData,
    LiveDataLoadMatch,
)
...
class LiveDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LiveDataEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> LiveData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.LiveData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
