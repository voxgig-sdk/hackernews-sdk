...
from hackernews_sdk.core.helpers import get_ctx_prop, to_map
...
class HackernewsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, HackernewsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, HackernewsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, HackernewsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return HackernewsError(code, msg, self)
