...
The Lua SDK for the Hackernews API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Item()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `kids` | `table` | The ids of the item's comments, in ranked display order |
| `parent` | `number` | The comment's parent: either another comment or the relevant story |
| `parts` | `table` | A list of related pollopts, in display order |
...
| Field | Type | Description |
| --- | --- | --- |
| `items` | `table` | Array of item IDs that have been updated |
| `profiles` | `table` | Array of usernames whose profiles have been updated |
...
| `submitted` | `table` | List of the user's stories, polls and comments |
...
```
lua/
├── hackernews_sdk.lua    -- Main SDK module
...
The main module (`hackernews_sdk`) exports the SDK constructor
