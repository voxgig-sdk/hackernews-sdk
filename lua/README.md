...
The Lua SDK for the Hackernews API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Item(nil):list(nil, nil)
...
local result, err = client:Hackernews(nil):load(
  { id = "test01" }, nil
)
