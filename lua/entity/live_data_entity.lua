...
local LiveDataEntity = {}
LiveDataEntity.__index = LiveDataEntity


function LiveDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LiveDataEntity)
  self._name = "live_data"
  self._client = client
...
function LiveDataEntity:get_name()
  return self._name
end


function LiveDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LiveDataEntity.new(self._client, opts)
end
...
function LiveDataEntity:mark_deleted()
  self._deleted = true
end


function LiveDataEntity:deleted()
  return true == self._deleted
end


function LiveDataEntity:data_set(args)
  if args ~= nil then
...
function LiveDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LiveDataEntity:match_set(args)
  if args ~= nil then
...
function LiveDataEntity:match_get()
...
function LiveDataEntity:stream(action, args, callopts)
...
---@param reqmatch LiveDataLoadMatch
---@param ctrl? table
---@return LiveData
---@return string? err
function LiveDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LiveDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
