...
local StoryEntity = {}
StoryEntity.__index = StoryEntity


function StoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StoryEntity)
  self._name = "story"
  self._client = client
...
function StoryEntity:get_name()
  return self._name
end


function StoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StoryEntity.new(self._client, opts)
end
...
function StoryEntity:mark_deleted()
  self._deleted = true
end


function StoryEntity:deleted()
  return true == self._deleted
end


function StoryEntity:data_set(args)
  if args ~= nil then
...
function StoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StoryEntity:match_set(args)
  if args ~= nil then
...
function StoryEntity:match_get()
...
function StoryEntity:stream(action, args, callopts)
...
---@param reqmatch StoryListMatch
---@param ctrl? table
---@return Story[]
---@return string? err
function StoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
