...
function ItemEntity.new(client, entopts)
  entopts = entopts or {}
...
function ItemEntity:get_name()
  return self._name
end


function ItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ItemEntity.new(self._client, opts)
end
...
function ItemEntity:mark_deleted()
  self._deleted = true
end


function ItemEntity:deleted()
  return true == self._deleted
end


function ItemEntity:data_set(args)
  if args ~= nil then
...
function ItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ItemEntity:match_set(args)
  if args ~= nil then
...
function ItemEntity:stream(action, args, callopts)
...
---@param reqmatch ItemListMatch
---@param ctrl? table
---@return Item[]
---@return string? err
function ItemEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
