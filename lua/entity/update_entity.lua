...
function UpdateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UpdateEntity)
  self._name = "update"
  self._client = client
...
function UpdateEntity:get_name()
  return self._name
end
...
function UpdateEntity:make()
  ...
  return UpdateEntity.new(self._client, opts)
end
...
function UpdateEntity:mark_deleted()
  self._deleted = true
end


function UpdateEntity:deleted()
  return true == self._deleted
end


function UpdateEntity:data_set(args)
  if args ~= nil then
...
function UpdateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UpdateEntity:match_set(args)
  if args ~= nil then
...
function UpdateEntity:match_get()
...
function UpdateEntity:stream(action, args, callopts)
...
function UpdateEntity:_run_op(ctx, post_done)
  local utility = self._utility
