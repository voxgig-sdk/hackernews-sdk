...
(0, node_test_1.describe)('ItemDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.HackernewsSDK({
    ...
    (0, node_test_1.test)('direct-list-item', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-item', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["item01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['item01'];
        }
        else {
            params.id = 'direct01';
        }
...
function liveScenariosActive() { return false && process.env.HACKERNEWS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.HackernewsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['HACKERNEWS_TEST_ITEM_ENTID'];
    ...
    const client = new __1.HackernewsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ItemDirect.test.js.map