...
//     (\`new HackernewsSDK(...)\` / \`HackernewsSDK.test(...)\` become
//     \`HackernewsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"item":{"test01":{"id":"test01"}},"live_data":{"test01":{"id":"test01"}},"story":{"test01":{"id":"test01"}},"update":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HackernewsSDK(...)\` and any
// \`HackernewsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HackernewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HackernewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
