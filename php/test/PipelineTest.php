...
{
    private static function utility(): HackernewsUtility
    {
        return new HackernewsUtility();
    }
    ...
    private static function ctx(array $over = []): HackernewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new HackernewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new HackernewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof HackernewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new HackernewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = HackernewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new HackernewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = HackernewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new HackernewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = HackernewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HackernewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = HackernewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new HackernewsResult([]),
        ]);
        [, $err] = HackernewsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new HackernewsSpec([]),
            'response' => null,
            'result' => new HackernewsResult([]),
        ]);
        [, $err] = HackernewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new HackernewsSpec(['step' => 's']),
            'response' => new HackernewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new HackernewsResult([]),
        ]);
        [, $err] = HackernewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new HackernewsSpec(['step' => 's']),
            'response' => new HackernewsResponse(self::resp(200, ['v' => 1])),
            'result' => new HackernewsResult([]),
        ]);
        [, $err] = HackernewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new HackernewsSpec(['step' => 's']),
            'response' => new HackernewsResponse(self::resp(200, ['v' => 2])),
            'result' => new HackernewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        HackernewsMakeResponse::call($ctx);
    ...
    {
        $preset = new HackernewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new HackernewsSpec([]),
            'response' => new HackernewsResponse([]),
            'result' => new HackernewsResult([]),
        ...
        [$got, $err] = HackernewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new HackernewsResult([])]);
        [, $err] = HackernewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new HackernewsSpec([]), 'result' => null]);
        [, $err] = HackernewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HackernewsSpec(['step' => 's']),
            'result' => new HackernewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new HackernewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HackernewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HackernewsSpec(['step' => 's']),
            'result' => new HackernewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new HackernewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HackernewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HackernewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new HackernewsSpec([]),
            'result' => new HackernewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = HackernewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HackernewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new HackernewsError('boom', 'boom');
        $utility->fetcher = function (HackernewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HackernewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = HackernewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (HackernewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = HackernewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(HackernewsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        HackernewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (HackernewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HackernewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = HackernewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new HackernewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new HackernewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = HackernewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = HackernewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = HackernewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new HackernewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, HackernewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new HackernewsResult(['ok' => false])]);
        $this->expectException(HackernewsError::class);
        HackernewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new HackernewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', HackernewsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        HackernewsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new HackernewsBaseFeature();
        $b = new HackernewsBaseFeature();
        HackernewsFeatureAdd::call($ctx, $a);
        HackernewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): HackernewsBaseFeature
    {
        $f = new HackernewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        HackernewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): HackernewsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new HackernewsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HackernewsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        HackernewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new HackernewsResponse(['headers' => null]),
            'result' => new HackernewsResult([]),
        ]);
        HackernewsResultHeaders::call($ctx);
