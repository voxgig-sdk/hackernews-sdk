...
class FtCtrl extends HackernewsControl
{
...
class FtClient
{
    ...
    private HackernewsUtility $utility;

    public function __construct(HackernewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): HackernewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HackernewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HackernewsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HackernewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HackernewsConfig::make_feature($name);
    ...
    private static function buildUrl(HackernewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HackernewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HackernewsResponse($fetched);
        }
    ...
    private function fail(HackernewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HackernewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
