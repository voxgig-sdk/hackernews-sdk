...
class HackernewsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = HackernewsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = HackernewsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!HackernewsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, HackernewsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return HackernewsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = HackernewsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = HackernewsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = HackernewsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new HackernewsError($op . "_allow",
                "HackernewsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = HackernewsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new HackernewsError("graphql_error",
                "HackernewsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function LiveData($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_live_data === null) {
                $this->_live_data = new LiveDataEntity($this, null);
            }
            return $this->_live_data;
        }
        return new LiveDataEntity($this, $data);
    }
