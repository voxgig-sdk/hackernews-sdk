...
class HackernewsRetryFeature extends HackernewsBaseFeature
{
    ...
    public function init(HackernewsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HackernewsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HackernewsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HackernewsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
