...
class LiveDataEntity
  ...
  def make
    opts = @_entopts.dup
    LiveDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HackernewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [LiveData, Hash] the current LiveData data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HackernewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of LiveData fields)
  def match_get
  ...
  #
  # @param reqmatch [LiveDataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.LiveData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [LiveData, Hash] the loaded LiveData; raises HackernewsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HackernewsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
