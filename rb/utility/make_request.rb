# Hackernews SDK utility: make_request
...
module HackernewsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HackernewsResponse.new({})
    ...
    if fetch_err
      ...
      response = HackernewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HackernewsResponse.new(fetched)
    else
