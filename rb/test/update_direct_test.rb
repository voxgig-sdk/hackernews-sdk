...
class UpdateDirectTest < Minitest::Test
  def test_direct_list_update
    setup = update_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-update", setup[:live] ? "live" : "unit")
    if _should_skip
...
def update_direct_setup(mockres)
