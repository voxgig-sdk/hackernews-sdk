...
class StoryDirectTest < Minitest::Test
  def test_direct_list_story
    setup = story_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-story", setup[:live] ? "live" : "unit")
    if _should_skip
...
def story_direct_setup(mockres)
