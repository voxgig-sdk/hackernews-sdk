...
class UpdateEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HACKERNEWS_TEST_UPDATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    update_ref01_data = nil
    if update_ref01_data_raw.length > 0
...
def update_basic_setup(extra)
  ...
  entid_env_raw = ENV["HACKERNEWS_TEST_UPDATE_ENTID"]
