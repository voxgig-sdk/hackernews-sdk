// Command Hackernews-cli is a CLI + REPL for the Hackernews SDK,
...
const prompt = "hackernews"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "item live_data story update user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HACKERNEWS_APIKEY (api key), HACKERNEWS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HACKERNEWS_APIKEY carries the API key and
	// HACKERNEWS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HACKERNEWS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HACKERNEWS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHackernewsSDK(opts)
