...
boru-driven command-line client **and** interactive REPL for the Hackernews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hackernews-cli)
make build
...
./hackernews-cli list item
./hackernews-cli list live_data

# 5. Override the API base URL for a single call
HACKERNEWS_BASE=https://api.example.com ./hackernews-cli list item
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hackernews-cli
   ```
...
Run with no arguments to open a REPL (prompt `hackernews>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `item`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hackernews-cli`. |
...
(`./hackernews-cli <expr>`) and interactively (the REPL), and expressions compose the
