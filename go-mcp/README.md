...
[MCP](https://modelcontextprotocol.io) server exposing the Hackernews SDK as
two agent tools — `hackernews_list` and `hackernews_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hackernews-mcp)
make build

# 2. Provide credentials via the environment
export HACKERNEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hackernews \
  -- /absolute/path/to/hackernews-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hackernews-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hackernews_list: first page of records
{ "entity": "item" }
{ "entity": "item", "query": { } }

// hackernews_load: one record by id
{ "entity": "live_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hackernews-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hackernews \
     -- "$PWD"/dist/*/hackernews-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hackernews_list` and `hackernews_load` tools now appear
   in new sessions. Ask the agent to *"list item using hackernews"*
   and it calls `hackernews_list` with `{"entity":"item"}`.
...
```sh
export HACKERNEWS_APIKEY=sk_live_xxx            # API key
export HACKERNEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `hackernews_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hackernews_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HACKERNEWS_APIKEY` | API key sent with every request. |
| `HACKERNEWS_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

item | live_data | story | update | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./hackernews-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hackernews_load","arguments":{"entity":"live_data","query":{"id":1}}}}'
```
