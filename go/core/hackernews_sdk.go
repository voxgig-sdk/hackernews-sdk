...
func (sdk *HackernewsSDK) OptionsMap() map[string]any {
...
func (sdk *HackernewsSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *HackernewsSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *HackernewsSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *HackernewsSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *HackernewsSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *HackernewsSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("HackernewsSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *HackernewsSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *HackernewsSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("HackernewsSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
