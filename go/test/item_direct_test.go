...
func TestItemDirect(t *testing.T) {
	t.Run("direct-list-item", func(t *testing.T) {
		setup := itemDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-item", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"item01"} {
...
func itemDirectSetup(mockres any) *itemDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HACKERNEWS_TEST_ITEM_ENTID"]; ok {
