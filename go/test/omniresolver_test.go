...
import (
	...
	omni "github.com/voxgig-sdk/hackernews-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HackernewsError); is {
				out := map[string]any{
					"name":    "HackernewsError",
...
func omniCtx(arg any, client *sdk.HackernewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HackernewsSDK); is {
			client = live
