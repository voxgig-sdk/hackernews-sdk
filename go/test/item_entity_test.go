...
func TestItemEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Item(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Item(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HACKERNEWS_TEST_ITEM_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		itemRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.item")))
...
func itemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HACKERNEWS_TEST_ITEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HACKERNEWS_TEST_ITEM_ENTID"])
