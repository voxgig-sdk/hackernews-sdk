...
func TestStoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HACKERNEWS_TEST_STORY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_, storyRef01ListOk := storyRef01ListResult.([]any)
		if !storyRef01ListOk {
...
func storyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HACKERNEWS_TEST_STORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HACKERNEWS_TEST_STORY_ENTID"])
